...
Vector3 MyMath::Cross(const Vector3& v1, const Vector3& v2) { return {v1.y * v2.z - v1.z * v2.y, v1.z * v2.x - v1.x * v2.z, v1.x * v2.y - v1.y * v2.x}; }
...
Vector3 MyMath::Lerp(const Vector3& start, const Vector3& end, float t) { return {Lerp(start.x, end.x, t), Lerp(start.y, end.y, t), Lerp(start.z, end.z, t)}; }
...
void MyMath::Accelerate(Vector3& velocity, const Vector3& direction, float acceleration, float deltaTime) { velocity = Add(velocity, Multiply(Normalize(direction), acceleration * deltaTime)); }
...
Vector3 MyMath::CalcGravityForce(float mass, float gravityAcceleration) { return {0.0f, -mass * gravityAcceleration, 0.0f}; }
