...
void SceneManager::SwitchBgm(SceneName sceneName) {
	...
	if (isBgmPlaying_ && shouldPlay && currentBgmHandle_ == nextBgmHandle) {
		return;
	}
	if (isBgmPlaying_) {
		audio_->StopWave(currentBgmVoiceHandle_);
...
void SceneManager::UpdateBgmVolume() {
	if (!scene_ || !isBgmPlaying_) {
		return;
	}

	const float volumeScale = scene_->GetBgmVolumeScale();
	if (volumeScale == currentBgmVolumeScale_) {
		return;
	}
...
void SceneManager::Update() {
	...
	// 次のシーンへ遷移
	if (!hasPendingScene_) {
		StartTransition(scene_->NextScene());
	}
}
...
void SceneManager::DrawFade() {
	if (fadeState_ == FadeState::None || !fadeSprite_) {
		return;
	}
